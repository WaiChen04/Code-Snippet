...
public class RUHungry {
     ...
     */
     ...
     public void menu(String inputFile) {
        ...
        categoryVar = new String[numCat];
        menuVar = new MenuNode[numCat];
        ...
        String nameCat = StdIn.readLine();

        categoryVar[k] = nameCat;
        ...
        String nameDish = StdIn.readLine();

        int numIngID = StdIn.readInt();

        int[] ingIDArray = new int[numIngID];
        
        for(int j = 0; j< numIngID ; j++){
            int addIngID = StdIn.readInt();
             ingIDArray[j] = addIngID;
            
        }
        Dish addDish = new Dish(nameCat,nameDish,ingIDArray);
        if(menuVar[k] == null){
            MenuNode addMenuNode = new MenuNode(addDish, null);
            menuVar[k] = addMenuNode;
        } else{
        
       MenuNode oldFront = menuVar[k];
MenuNode addMenuNode = new MenuNode(addDish,oldFront);
        menuVar[k] = addMenuNode;
    ...
    int hashcode = ingredientID %stockVarSize;

    if(stockVar[hashcode] == null){
        stockVar[hashcode] = newNode;
    } else{
        
        StockNode oldFront = stockVar[hashcode];
        newNode.setNextStockNode(oldFront);
        stockVar[hashcode] = newNode;
    }
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        if(stockVar[hashcode] != null){
            StockNode ptr = stockVar[hashcode];
                while(ptr != null){
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if(ingredientName == null){
            ...
            int currentStock = found.getIngredient().getStockLevel();

            int newStock = currentStock + stockAmountToAdd;

            found.getIngredient().setStockLevel(newStock);
        }
        if(ingredientID == -1){
    ...
    public void updatePriceAndProfit() {
        ...
        for(int i = 0; i <menuVar.length; i++){
            MenuNode current = menuVar[i];

            while(current != null){
            Dish dish = current.getDish();
            double cost = 0;
            int [] ingredientIDsArray = dish.getStockID();
            for (int j = 0; j < ingredientIDsArray.length; j++){
                StockNode stock = findStockNode(ingredientIDsArray[j]);
                if(stock != null){
                double addCost = stock.getIngredient().getCost();
                cost = cost + addCost;
                }
            }
    ...
    public void createStockHashTable (String inputFile){
        ...
        for(int i = 0; i <51; i++){
            ...
            StockNode addNode = new StockNode(ingredient, null);

            addStockNode(addNode);
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        TransactionNode addTransactionNode = new TransactionNode(data,null);

        if(transactionVar == null){
            transactionVar = addTransactionNode;
        }else{
        ...
        ptr.setNext(addTransactionNode);
    ...
    public boolean checkDishAvailability ( String dishName, int numberOfDishes ){
        ...
        MenuNode nodeOfdish = findDish(dishName);

        Dish dish = nodeOfdish.getDish();

        int [] ingredIds = dish.getStockID();

        for(int i = 0; i < dish.getStockID().length ; i++){
            StockNode ingredStock = findStockNode(ingredIds[i]);
            Ingredient ingredient = ingredStock.getIngredient();
    ...
    public void order ( String dishName, int quantity ){
        ...
        if(checkDishAvailability(dishName, quantity) == true){
            ...
            String realDishName = dish.getDishName();
            TransactionData order = new TransactionData("order", realDishName, quantity, profit , true);
        ...
        for(int i = 0; i < dish.getStockID().length ; i++){
        updateStock(null, ingredIds[i], -quantity);
        }
        } else{
            ...
            MenuNode bestNext = findDish(dishName).getNextMenuNode();
            MenuNode ptr = bestNext;
            if(ptr == null){
                String cat = current.getDish().getCategory();
                        int categoryId = findCategoryIndex(cat);
                        ptr = menuVar[categoryId];
                        bestNext = ptr;
            }
            do{
                Dish dish = ptr.getDish();

                String newdishName = dish.getDishName();
                if(checkDishAvailability(newdishName, quantity) == true){
            double newprofit = (dish.getProfit())*quantity;
            TransactionData neworder = new TransactionData("order", newdishName, quantity, newprofit , true);
        ...
        updateStock(null, ingredIds[i], -quantity);
            ...
            newdishName = dish.getDishName();
                    
            TransactionData neworder = new TransactionData("order", newdishName, quantity, 0 , false);
    ...
    public double profit () {
        ...
        while(ptr != null){
            ...
            double addprofit = ptr.getData().getProfit();
    ...
    public void donation ( String ingredientName, int quantity ){
        ...
        StockNode selectStockNode = findStockNode(ingredientName);
        int stocklevel = selectStockNode.getIngredient().getStockLevel();

        if(profit > 50.0 && quantity <= stocklevel){
            TransactionData data = new TransactionData("donation", ingredientName, quantity, 0.0, true);
            addTransactionNode(data);
            updateStock(ingredientName, -1, -quantity);

        }
        else{
            TransactionData data = new TransactionData("donation", ingredientName, quantity, 0.0, false);
            addTransactionNode(data);
        }
    ...
    public void restock ( String ingredientName, int quantity ){
        ...
        StockNode ingredStock = findStockNode(ingredientName);

        double cost = ingredStock.getIngredient().getCost() * quantity;
